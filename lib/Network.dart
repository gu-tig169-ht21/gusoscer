...
class Api {
  ...
  static Future delete_ActivityApi(String activityId) async {
    var response = await http.delete(Uri.parse(
        '$API_URL/todos/$activityId?key=$API_KEY&_confirm=true')); //ta bort kolon? kanske också confirmtrue..
  ...
  static Future put_ActivityApi(Todo activity) async {
    ...
    var response =
        await http.put(Uri.parse('$API_URL/todos/$activityId?key=$API_KEY'),
        body: bodyString,
        headers: {'Content-Type': 'application/json'},
      );
    bodyString = response.body;
  ...
  static Future<List<Todo>> getActivitesApi() async {
    ...
    var bodyString =
        response.body; //kanske inte ska va en string? eftersom bool också?
